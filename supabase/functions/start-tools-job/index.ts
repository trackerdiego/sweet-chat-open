...
const TOOL_PROMPTS: Record<string, { system: (ap: Record<string, unknown>, niche: string, style: string) => string; user: (input: string, niche: string) => string }> = {
  ...
  reelsDescription: {
    system: (ap, niche, style) => `Você é Master Copywriter de Instagram especializado em legendas de Reels para "${niche}".\n${PT_BR_RULES}\nEstilo: ${style}.\n\nPERFIL DO PÚBLICO:\nAvatar: ${ap.avatar || ''}\nFeridas: ${JSON.stringify(ap.coreWounds || [])}\nDesejo oculto: ${ap.deepOccultDesire || ''}\nGatilhos verbais: ${JSON.stringify(ap.verbalTriggers || [])}\nRelatabilidade: ${JSON.stringify(ap.everydayRelatability || [])}\nObjeções: ${JSON.stringify(ap.objections || [])}\n\nREGRAS DURAS:\n- 1ª linha É o hook que segura o scroll (curiosidade, dor ou promessa). Sem "Você sabia que...".\n- Máximo 2200 caracteres no fullCaption.\n- Quebras de linha curtas (mobile-first), parágrafos de 1-3 linhas.\n- Emojis com PROPÓSITO (ancorar emoção/listar), nunca decoração.\n- CTA conversacional no fim (pede comentário/save/compartilhar).\n- Hashtags: 8-15 misturando 3 amplas + 5 do nicho + 3-5 cauda longa. SEM o caractere #.\n- 3 hooks alternativos pra A/B test (variações do hookLine).\n- Hashtags em minúsculas, sem acento e sem espaço (ex.: "marketingdigital", "dicasdebeleza").`,
    user: (input, niche) => `Conteúdo do Reel (transcrição ou tema) para "${niche}":\n\n${input}\n\nGere a legenda completa pronta pra colar no Instagram + 3 hooks alternativos + hashtags estratégicas. Revise ortografia e concordância antes de devolver.`,
  },
};
