...
console.log("[audience-profile] boot — using native endpoint, responseSchema");
...
serve(async (req) => {
  ...
  try {
    ...
    // Step 1: descrição do público (texto livre, gemini-2.5-pro pra raciocínio)
    ...
    try {
      ...
      if (e instanceof GeminiError) {
        ...
        const msg = e.status === 429 ? "Limite de requisições excedido. Tente novamente." : "Serviço de IA instável. Tente novamente em 1-2 minutos.";
    ...
    // Step 2: avatar visceral (responseSchema, gemini-2.5-pro)
    ...
    if (upsertError) console.error("Error saving audience profile:", upsertError);

    return new Response(JSON.stringify({ audienceDescription, avatarProfile }), {
